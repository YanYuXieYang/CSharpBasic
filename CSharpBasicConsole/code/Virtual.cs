...
namespace CSharpBasic.code
{
    ...
    class Virtual
    {
        public Virtual()
        {
            ...
            d.Func();    // 执行d.Func：1.先检查申明类A 2.检查到是虚拟方法 3.转去检查实例类D，无重载的（这个地方要注意了，虽然D里有实现Func()，
                // 但没有使用override关键字，所以不会被认为是重载） 4.转去检查类D的父类A，就为本身 5.执行父类A中的Func方法 5.输出结果 Func In A
            D d1 = new D();
            d1.Func(); // 执行D类里的Func()，输出结果 Func In D
            A ad = d1; // 与对象d相等
            ad.Func();  // 输出结果 Func In A

            B b1 = new B(); // 输出结果 Func In B
            b1.Func();
        }
        class A
        {
